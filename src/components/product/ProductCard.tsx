  ...
  return (
    <div className="group relative flex flex-col">
      ...
      <div className="flex flex-col space-y-1.5 px-1">
        ...
        <Link href={`/product/${slug}`} className="font-bold text-[13px] md:text-[14px] text-[#0A0A0A] tracking-wide uppercase hover:text-[#171717]/70 transition-colors line-clamp-2">
          {product.name}
        </Link>
        <div className="text-[14px] md:text-[15px] font-medium flex items-center gap-2 pt-1">
          {crossedOutPrice ? (
            <>
              ...
              <span className="text-[#171717]/40 line-through text-[11px] md:text-xs">₹{crossedOutPrice.toLocaleString('en-IN')}</span>
            </>
          ) : (
            <span className="text-[#0A0A0A] font-black">₹{currentPrice.toLocaleString('en-IN')}</span>
          )}
        </div>
      </div>
    </div>
